...
# configuration for windows, to generate packaging as the daily build
...
`configure' configures LibreOffice 5.0.0.0.beta3+ to adapt to many kinds of systems.
...
Optional Features:
  ...
  --enable-pch            Enables precompiled header support for C++.

  --enable-win-mozab-driver
                          LibreOffice includes a driver to connect to Mozilla
                          address books under Windows, to build with this
  ...
  --enable-lto            Enable link-time optimization. Suitable for
                          ...
                          for speed. (For GCC, best to use GCC 4.6 or later
                          and 'gold' linker. Works fine (and is turned on
                          automatically in an optimising build) for MSVC,
                          otherwise experimental work in progress that
                          shouldn't be used unless you are working on it.)
  ...
  --enable-online-update  Enable the online update service that will check for
                          ...
                          enabled on Windows and Mac, disabled on Linux.
  ...
  --enable-library-bin-tar
                          ...
                          identified by the source tarbal, the content of the
...
Optional Packages:
  ...
  --with-system-npapi-headers
                          Use NPAPI headers provided by system instead of
                          bundled ones. Used in extensions/source/plugin
  ...
  --with-visual-studio=2013
                          ...
                          several are installed. If not specified, the order
                          of preference is 2013 (including Express editions).
                          Not very useful at the moment, as currently only
                          2013 is supported, anyway.

                          Usage:     --with-visual-studio=2013

  --with-windows-sdk=<7.1(A)/8.0(A)/8.1(A)>
...
Some influential environment variables:
  ...
  NPAPI_HEADERS_CFLAGS
              C compiler flags for NPAPI_HEADERS, overriding pkg-config
  NPAPI_HEADERS_LIBS
              linker flags for NPAPI_HEADERS, overriding pkg-config
  GRAPHITE_CFLAGS
