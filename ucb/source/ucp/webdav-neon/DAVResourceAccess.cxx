...
{
    SAL_WARN_A("debuglogger.webdav","DAVAuthListener_Impl::authenticate - called");
    if ( m_xEnv.is() )
    {
            ...
            if ( xSelection.is() )
            {
                ...
                if ( !xAbort.is() )
                {
                    ...
                    if ( bUseSystemCredentials )
                    {
                        ...
                        inoutUserName.clear();
                        outPassWord.clear();
                        SAL_WARN_A("debuglogger.webdav","DAVAuthListener_Impl::authenticate: System credentials where selected");
                    }
                    else
                    {
                    ...
                    SAL_WARN_A("debuglogger.webdav","DAVAuthListener_Impl::authenticate: Abort was selected ");
            }
            else
                SAL_WARN_A("debuglogger.webdav","DAVAuthListener_Impl::authenticate: there is NO ucbhelper::InteractionContinuation (xSelection)");
        }
        else
            SAL_WARN_A("debuglogger.webdav","DAVAuthListener_Impl::authenticate: there is NO task::XInteractionHandler");
    }
    else
        SAL_WARN_A("debuglogger.webdav","DAVAuthListener_Impl::authenticate: there is NO  XCommandEnv");
    // Abort.
    return -1;
}
