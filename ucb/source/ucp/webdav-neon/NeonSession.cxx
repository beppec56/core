 ...
 * ignored.)  */
    ...
    SAL_WARN_A("ucb.ucp.webdav","NeonSession_NeonAuth - called");
    ...
    {
        SAL_WARN_A( "ucb.ucp.webdav", "NeonSession_NeonAuth - username to long!" );
        return -1;
    }
    ...
    {
        SAL_WARN_A( "ucb.ucp.webdav", "NeonSession_NeonAuth - password to long!" );
        return -1;
    }
...
{
    SAL_WARN_A("ucb.ucp.webdav","NeonSession_CertificationNotify called");
    OSL_ASSERT( cert );
    ...
    throw( std::exception )
{
    ...
    osl::Guard< osl::Mutex > theGuard( m_aMutex );
    SAL_WARN_A( "ucb.ucp.webdav", "OPTIONS - relative URL <" << inPath << ">" );
    ...
    if ( theRetVal == NE_OK )
    {
                      ...
                      req, cursor, &name, &value ) ) != NULL )
        {
            ...
            SAL_WARN_A( "ucb.ucp.webdav", " " << aHeaderName << ":" << aHeaderValue );
...
//#if defined SAL_LOG_INFO
    { //debug
        ...
        SAL_INFO_A( "ucb.ucp.webdav", " property requested:" );
...
{
    osl::Guard< osl::Mutex > theGuard( m_aMutex );
    SAL_WARN_A( "ucb.ucp.webdav", "PROPFIND - relative URL: <" << inPath << "> Depth: " << inDepth );
...
//#if defined SAL_LOG_INFO
    { //debug
        ...
        {
            SAL_INFO_A( "ucb.ucp.webdav", " returned property (names only):" );
    ...
    for ( n = 0; n < nPropCount; ++n )
    {
        ...
        if ( rValue.operation == PROPSET )
        {
            ...
            else if ( !( rValue.value >>= aStringValue ) )
            {
                ...
                {
                    SAL_WARN_A( "ucb.ucp.webdav", "Unsupported type!" );
                    // Error!
                    pItems[ n ].value = nullptr;
                    theRetVal = NE_ERROR;
                    nPropCount = n + 1;
...
void NeonSession::DESTROY( const OUString & inPath,
    ...
    throw ( std::exception )
{
    SAL_WARN_A("ucb.ucp.webdav","DESTROY - relative URL <" << inPath << ">" );
    osl::Guard< osl::Mutex > theGuard( m_aMutex );
...
namespace
{
                                              ...
                                              int timeout )
    {
        ...
        if ( timeout != NE_TIMEOUT_INFINITE )
        {
            ...
            {
                SAL_WARN_A( "ucb.ucp.webdav", "No chance to refresh lock before timeout!" );
            }
        }
...
{
    osl::Guard< osl::Mutex > theGuard( m_aMutex );
    SAL_WARN_A( "ucb.ucp.webdav", "LOCK (create) - relative URL <" << inPath << ">" );
    ...
    if ( theRetVal == NE_OK )
    {
        ...
        SAL_INFO_A( "ucb.ucp.webdav", "Created lock for <" << makeAbsoluteURL( inPath )
                  << "> token: " << theLock->token );
    }
    else
    {
        ...
        SAL_WARN_A( "ucb.ucp.webdav", "Obtaining lock for <"
...
//#if defined SAL_LOG_INFO
    {
        ...
        SAL_WARN_A( "ucb.ucp.webdav", "LOCK (refresh) - relative URL: <" << p << ">" );
        ne_free( p );
    }
//#endif
    ...
    if ( ne_lock_refresh( m_pHttpSession, pLock ) == NE_OK )
    {
        ...
        SAL_INFO_A( "ucb.ucp.webdav", "Lock successfully refreshed." );
        return true;
    }
    else
    {
        SAL_WARN_A( "ucb.ucp.webdav", "Lock not refreshed!" );
        return false;
    }
}
...
{
    osl::Guard< osl::Mutex > theGuard( m_aMutex );
    SAL_WARN_A( "ucb.ucp.webdav", "UNLOCK - relative URL <" << inPath << ">" );
    ...
    if ( !theLock )
    {
        SAL_WARN_A("ucb.ucp.webdav","DAVException::DAV_NOT_LOCKED issued");
    ...
    {
        SAL_WARN_A( "ucb.ucp.webdav", "Unlocking of "
                  << makeAbsoluteURL( inPath ) << "> failed." );
    }
...
//#if defined SAL_LOG_INFO
    {
        char * p = ne_uri_unparse( &(pLock->uri) );
        SAL_WARN_A( "ucb.ucp.webdav", "UNLOCK (from store) - relative URL: <" << p << ">" );
        ne_free( p );
    }
//#endif
    ...
    {
        SAL_INFO_A( "ucb.ucp.webdav", "UNLOCK succeeded." );
        return true;
    }
    else
    {
        SAL_WARN_A( "ucb.ucp.webdav", "UNLOCK failed!" );
        return false;
    }
}
    ...
    throw ( std::exception )
{
    ...
    {
        case NE_OK:
            SAL_WARN_A("ucb.ucp.webdav","No error on NEON");
            return;
        ...
        case NE_REDIRECT:
        {
            ...
            SAL_WARN_A("ucb.ucp.webdav","DAVException::DAV_HTTP_REDIRECT: URI: "<<aUri.GetURI());
            throw DAVException(
                      ...
                      void * userdata )
{
    ...
    if ( getheaders )
    {
        SAL_WARN_A( "ucb.ucp.webdav"," GET - Headers:" );
        void *cursor = nullptr;
        const char *name, *value;
        while ( ( cursor = ne_response_header_iterate(
                               req, cursor, &name, &value ) ) != nullptr )
        {
            ...
            SAL_WARN_A( "ucb.ucp.webdav","  " << name << ":" << value );
