...

    {
        ...
        if ( m_xCachedProps.get() )
        {
            ...
            if ( xCachedProps->containsAllNames( rProperties, aMissingProps ) )
            {
                ...
                bHasAll = true;
                {//debug
                    SAL_INFO_A("ucb.ucp.webdav","All Props in cache!");
                }
             }
                            ...
                            {//debug
                                ...
                                while ( it != end )
                                {
                                    ...
                                    if( (*it).Value >>= aPropValue )
                                        SAL_INFO_A( "ucb.ucp.webdav", "PROPFIND (getPropertyValues) - returned property: " << (*it).Name << ":" << aPropValue );
                                    else if( (*it).Value >>= bValue )
                                        SAL_INFO_A( "ucb.ucp.webdav", "PROPFIND (getPropertyValues) - returned property: " << (*it).Name << ":" <<
                                                  ( bValue ? "true" : "false" ) );
                ...
                aDAVOptions.isClass3() )
            {
                    ...
                    aDAVOptions.isClass3() )
                {
                    ...
                    try
                    {
                        ...
                        {
                            SAL_INFO_A( "ucb.ucp.webdav", "This WebDAV server has no supportedlock property, check for allowed LOCK method in OPTIONS" );
                            // TODO: if the lock type is changed from 'exclusive write' to 'shared write'
    ...
    throw ( uno::Exception, std::exception )
{
    ...
    else
    {
        ...
        if( aDAVOptions.isClass1() ||
            aDAVOptions.isClass2() ||
            aDAVOptions.isClass3() ) // at least class one is needed
        {
            try
            {
...
//#if defined SAL_LOG_INFO
                    {//debug
                        ...
                        while ( it != end )
                        {
                            ...
                            uno::Sequence< ucb::LockEntry > aSupportedLocks;
                            if((*it).Value >>= aPropValue )
                                SAL_INFO_A( "ucb.ucp.webdav", "PROPFIND (getResourceType) - ret'd prop: " << (*it).Name << ":" << aPropValue );
                            else if( (*it).Value >>= bValue )
                                SAL_INFO_A( "ucb.ucp.webdav", "PROPFIND (getResourceType) - ret'd prop: " << (*it).Name << ":" <<
                                          ( bValue ? "true" : "false" ) );
                            else if( (*it).Value >>= aSupportedLocks )
                            {
                                SAL_INFO_A( "ucb.ucp.webdav", "PROPFIND (getResourceType) - ret'd prop: " << (*it).Name << ":" );
                                for ( sal_Int32 n = 0; n < aSupportedLocks.getLength(); ++n )
                                {
                                    SAL_INFO_A( "ucb.ucp.webdav","PROPFIND (getResourceType) -       supportedlock[" << n <<"]: scope: "
                                              << ( aSupportedLocks[ n ].Scope ? "shared":"exclusive" )
                                              << ", type: "
                                              << ( aSupportedLocks[ n ].Type ? "" : "write" ) );
                                }
...
//#if defined SAL_LOG_INFO
            { //debug
                ...
                SAL_INFO_A( "ucb.ucp.webdav", "DAVOptions for URL <" << rDAVOptions.getURL()
